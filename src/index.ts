...
app.post(
  ...
  zValidator(
    "json",
    pagination.omit({ results: true }).extend({
      fr: z.string().optional(),
      oldestTweetId: z.string().optional(),
      p: z.string(),
...
app.post("/recommend/video", zValidator("json", pagination), async (c) =>
  c.json(await yahoo.getRecommendVideo(c.req.valid("json")))
);

app.post(
  "/interest/category/pagination/:categoryId",
  zValidator("param", z.object({ categoryId: z.string() })),
  zValidator("json", z.strictObject({ oldestTweetId: z.string().optional() })),
  async (c) =>
    c.json(
      await yahoo.getInterestCategoryPagination(
